...
| `ide_change_signature` | Preview or change a method signature with automatic caller updates (Java, Kotlin JVM functions) | Disabled |
...
The server retains an IntelliJ `SmartPsiElementPointer` and checks the original file identity. The ID is
...
obtain a new ID. Deleting and recreating a file at the same path expires its old handles.
Definition and metadata lookups preserve the stored target, including distinct JVM accessors;
generated declarations without their own source text use a source-context preview.
Successful refactorings return current `updatedSymbol` metadata with the same ID
