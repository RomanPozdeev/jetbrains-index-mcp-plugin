...
- The four legacy top-level analysis fields above apply to single-file calls. Multi-file calls return one aggregate `problems` list and `fileAnalyses: [{file, mode, fresh, timedOut, message}]`, including an entry for a missing, deleted, failed, or unprocessed requested file.
...
project and MCP session. They use an independent 128-entry access-order LRU and expire after ten
minutes without access. An expired, evicted, wrong-project, wrong-tool, or invalidated cursor must
