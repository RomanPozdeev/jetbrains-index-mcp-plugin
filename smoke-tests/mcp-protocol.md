...
1. `gw ./gradlew buildPlugin` — produces `build/distributions/<name>-<version>.zip`
...
- Call with no `path`, `project_path` pointing to this project (after `gw ./gradlew buildPlugin`)
...
- FAIL (no zip): "No plugin zip found" — run `gw ./gradlew buildPlugin` first
