...
- Add opt-in bounded hierarchy pagination with parent/depth identity and session-scoped cursors; preserve legacy trees and limits, return completed work at continuation budgets, and propagate cancellation from reflective collectors. Keep Kotlin accessors distinct, bound JS direct-caller discovery, identify TS parents by declaration, traverse excluded type intermediates, and keep every returned legacy handle resolvable. Share the legacy type handle budget across root and both directions, preserve input declaration IDs, expand distinct Kotlin getter/setter bodies, keep Java PSI optional, and bridge excluded intermediate types consistently in JS/TS, Python and PHP.
- Add structured file-outline nodes alongside formatted text, with exact declaration handles and explicit handle-budget metadata. Preserve Markdown heading handles during lookup and support previews of implicit enum methods; cover Java, Kotlin and TypeScript declaration kinds.
...
- Add change-signature dry-run previews with public usage/conflict discovery, stable symbol targets, and updated declaration metadata. Match native apply usage snapshots, allow trailing varargs without defaults, and accept already satisfied package-private visibility. Allow unchanged return types and override-only parameter additions, require defaults for generated delegates, and discover new external coordinate targets before PSI resolution.

- `ide_refactor_safe_delete` supports the shared non-mutating preview response and exact/nested symbol targets. Forced file deletion previews agree with apply even without declarations or complete usage discovery. Java parameter deletion ignores unrelated non-code text and handles lambda, catch and loop bindings without method-parameter assertions. Block isolated parameter deletion across overrides, resolve Kotlin light targets to source, and refuse apply when resource discovery fails. Reject synthetic Kotlin JVM methods that have no matching standalone source declaration, including implicit constructors, generated copy methods and property accessors.
...
- `ide_diagnostics` accepts a small `files` batch with one shared analysis deadline and per-file freshness and truncation metadata. Existing single-file and build/test-only requests remain supported. Isolate per-file analyzer failures and deduplicate resolved files without collapsing distinct symlink/parent paths. Preserve literal whitespace in requested filenames.
...
- **Kotlin change signature resolves source functions** — `ide_change_signature` accepts Kotlin JVM functions by position or `symbolId` for preview and apply. Override parameter changes start at the base declaration and update the interface, implementations, and callers. Preview preserves the original override handle and checks the actual base source file for writability; apply verifies fresh source PSI. A valid handle for the wrong declaration kind is no longer reported as expired.
...
- Use the supported Java lookup language in copyable qualified-target examples for JVM declarations.
...
- Reject hierarchy continuations whose retained declarations belong to a deleted and recreated file.
...
- `ide_sync_files` refreshes known deleted paths through their nearest existing parent and reports the actual refreshed roots. It validates the whole batch first; unknown missing paths return "Not found" without refreshing the project. Absolute paths, traversal and symlink escapes are rejected. Preserve registered symlink root paths when refreshing VFS and PSI. Resolve null/deleted routing roots and discover new paths with shallow ancestor refreshes, without recursively refreshing unrelated directories.
- Validate bind-host syntax before DNS lookup, including IDN hostnames, absolute DNS names, and IPv4-mapped IPv6 literals. Malformed labels remain invalid even with wildcard DNS. Persist, bind and restart with the same normalized IDN hostname; equivalent Unicode spelling does not trigger a needless restart. Keep unchanged scoped IPv6 settings usable when their saved network interface is offline.
