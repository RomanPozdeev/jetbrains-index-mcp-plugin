    ...
    internal fun bind(
        ...
        return serverEpoch.ifCurrent(
            ...
            synchronized(this) {
                ...
                entries[symbolId] = Entry(WeakReference(project), pointer, fileIdentity, expectedGeneration, now)
                counters.insertions++
                evictLruOverflow()
    ...
    fun resolve(project: Project, symbolId: String): Result<PsiElement> {
        ...
        val resolved = serverEpoch.ifCurrent(expectedGeneration, stale = { null }) {
            synchronized(this) {
                ...
                Triple(candidate, candidate.pointer, candidate.fileIdentity)
            }
        } ?: return expiredLookup(symbolId)
        val (entry, pointer, fileIdentity) = resolved
        ...
        if (element == null || !element.isValid || fileIdentity?.matches(pointer.virtualFile) == false) {
            synchronized(this) {
