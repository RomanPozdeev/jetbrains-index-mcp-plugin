...
10. **Use the right diagnostics scope**: use `file` for intentions or line/range filters; use mutually exclusive `files` for a small batch. A `files` request shares one timeout budget and returns aggregate `problems` plus per-path `fileAnalyses`. Use `ide_project_diagnostics` for broad, fail-closed project coverage.
11. **Inspect structured outlines when chaining**: `ide_file_structure.structure` is the compatible display string; prefer `nodes` for automation. Nodes include semantic kind, signature, modifiers, line range, children, and an optional ID bound to the exact PSI declaration. Kotlin kinds distinguish interface/class/enum/annotation/object; anonymous implementations have readable location names and no qualified name.
