...
**Pagination**: `calls` is a flat deterministic BFS page and `returnedNodes` excludes the root. Continue while `hasMore` is true. Hierarchy cursors are exact-session/project smart-pointer frontiers with a ten-minute inactivity TTL and separate 128-entry LRU. Expired/evicted/wrong-project/wrong-tool cursors require a fresh query. Stateless HTTP provides pages and response metadata, not reliable live progress.
...
**Returns**: `{ file, language, structure, nodes: [{name, kind, signature?, modifiers, line, endLine?, children, symbolId?}] }`
...
**Returns**: `{ problems: [{message, severity, file, line, column, endLine?, endColumn?}], intentions?, problemCount, intentionCount?, analysisFresh?, analysisTimedOut?, analysisMessage?, analysisMode?, fileAnalyses?: [{file, mode?, fresh, timedOut, message?}], buildErrors?, buildErrorCount?, buildWarningCount?, buildErrorsTruncated?, buildTimestamp?, testResults?, testResultsTruncated?, testSummary? }`
