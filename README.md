...
by a separate 128-entry LRU with a ten-minute inactivity TTL. The stateless HTTP transport does not
provide reliable live progress; consume pages until `hasMore` is false.
...
single-file only. `ide_sync_files` can target paths that have already been deleted: it refreshes the
...
names when available. Anonymous implementations have a readable location-based name such as
